                        ...
                        (1..).map(|k| (start_position as i32 + move_direction * k) as usize)
                    {
                        ...
                        if piece_moving == &Piece::Pawn {
                            ...
                            if (*move_direction == Direction::NORTH + Direction::WEST
                                ...
                                && destination_square == Square::Empty
                                && end_position != board_state.en_passant_position.unwrap_or(123456) // TODO remove hack
                                && end_position != board_state.king_passant_position.unwrap_or(123456)
                            // TODO remove hack
                            {
                                break;
                            }
...
pub fn rotated(board_state: &BoardState) -> BoardState {
    ...
    for coordinate in 0..BOARD_SIZE {
        new_board[coordinate] = match board_state.board[BOARD_SIZE - 1 - coordinate] {
    ...
    BoardState {
        ...
        en_passant_position: if board_state.en_passant_position.is_none() {
            None
        } else {
            Some(BOARD_SIZE - 1 - board_state.en_passant_position.unwrap())
        },
        king_passant_position: if board_state.king_passant_position.is_none() {
            None
        } else {
            Some(BOARD_SIZE - 1 - board_state.king_passant_position.unwrap())
        },
    }
}
...
pub fn after_move(board_state: &BoardState, move_: &(usize, usize)) -> BoardState {
    ...
    rotated(&BoardState {
        ...
        my_castling_rights: my_castling_rights,
        opponent_castling_rights: opponent_castling_rights,
        king_passant_position: king_passant_position,
...
pub fn move_value(board_state: &BoardState, move_: &(usize, usize)) -> i32 {
    ...
    // Score for captures
    match board_state.board[end_position] {
        Square::OpponentPiece(captured_piece) => {
